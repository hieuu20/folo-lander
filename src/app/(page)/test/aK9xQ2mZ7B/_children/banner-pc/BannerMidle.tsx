...
function animateStars(tl: gsap.core.Timeline, starElements: any[], stepIndex: number) {
    if (stepIndex === 0) {
        ...
        starElements.forEach((star, index) => {
            ...
            // if (stepIndex == 1) {
            //     console.log({ xFrom, yFrom, xnext: xFrom + stars[stepIndex].x[stepIndex], yNext: yFrom + stars[stepIndex].y[stepIndex], stepIndex })
            // }
            tl.fromTo(
                star,
                { x: xFrom, y: yFrom },
            ...
            >
                <Swiper
                    ...
                    onSlideChange={(swiper) => {
                        setCenterRealIndex(prev => centerRealIndex + 1);
                    }}
    ...
    const renderContent = () => {
        if (centerRealIndex == index) {
            return (
                <Flex
                    pos={"relative"}
                    direction={"column"} w={"100%"} align={"center"} c={"white"} className='aspect-[0.46284631701]'
                >
                    <Box
                        pos={"relative"}
                        w={"100%"}
                        className={twMerge(
                            'idol-image overflow-hidden rounded-t-[33px] w-full h-full',
                        )}
                    >
